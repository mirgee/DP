...
Investignating the significance of correlations between nonlinear measures and depression scores, we found that LLE, DFA, HE, HD, and SE are significantly correlated with depression score in some cortical areas. The strongest correlations was found for LLE. Although CD showed the weakest correlations with depression score per channel, using it in combination with other nonlinear measures or by combining a number of channels resulted in one of the best depression classifiers evaluated. For selection of channels relevant for classification, a genetic algorithm was far more successful that any other feature selection method evaluated.

In \cite{hosseinifard2013}, it was found that CD was the most useful, LLE least.

\subsection*{The Largest Lyapunov Exponents are Significantly Correlated with Positive Treatment Response}
Positively correlated with depression score, negatively with response. Thus LLE can be . We are not aware of any studies showing a similar result. Other nonlinear measures were not sign. corr., but still good feature for lin. classifiers.

\subsection*{Analysis of Spatial Distribution of Nonlinear Measures across Brain Regions in Depression}
Although some neuroscientific theories suggest that medical depression is caused by malfuction in the right hemisphere, we found nonlinear measures computed in both hemispheres contribute significantly to high depression score, with slightly stronger contribution from the left hemisphere.

Response: F3 and T6, depression T6.

\subsection*{Analysis of Nonlinear Measure and Input Parameter Estimation Algorithms and Procedures for EEG Analysis}
In the case of EEG and this problem, the evaluated nonlinear measure embedding parameter selection techniques are do not lead to most discriminative results, although they are widely used.

\subsection*{Evaluation of FBCSP-inspired Neural Network Architectures for Depression Diagnosis and Prognosis}
Deep learning technique did not work well on this particular problem. To use DL technique: dataset probably must contain more of \dots, or the the architecture  be improved my making use of our finding that NLM's.
