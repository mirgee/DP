...
Stationarity was evaluated on multiple time scales using the stationarity test described in Section \ref{sec:stationarity}. For results, see Table .
...
Typical plots of autocorrelation and delayed mutual information can be seen on Fig. - similar behavior was observed across channels and patients. As expected from construction of the surrogate data, their plots look identical to those for original data. Delayed mutual information lacks perceptible minima, so this test yields no useful or reliable estimates of $\tau$. The red line on the autocorrelation plot shows the value $1-1/e$. Delays suggested by this test suggest values of $\tau$ in range 2-4.

As expected, the reconstructed attractors for small delays cluster along the main diagonal, and get increasingly random with larger $\tau$. However, it is difficult to judge on the degree of folding in those reconstruction from these plots, mainly due to large number of data samples.
...
After some experimentation, the parameters for FNN were $A_{\text{tol}} = 0.5$ and $R_{\text{tol}} = 3$. (TODO: Plot for different values.) The percentage of FNN's identified by the first te almost always plateaus for about $m=10$. The second test, however, is more sensitive to the tolerance value, and percentage of false neighbors reported by this test first quickly drops, and then starts raising. This behavior suggests noise corruption. \cite{kennel1992determining} As suggested by Kennel et al. \cite{kennel1992determining}, we used both criteria in conjuction to obtain the optimal value as suggested by FNN algorithm. (TODO: Report average $m$ computed this way)

The $E_1$ statistic of AFN usually stops increasing for the same value as reported by FNN, thus the results of those two algorithms are in accordance. From Fig. , we can see that the $E_2$ statistic, which should be used for distinguishing stochastic and deterministic data \cite{Cao1997}, behaves almost identically for both original and surrogate time series'. This is an indication of nondeterminism of our data. 

However, this effect is amplified for $\tau > 1$, as the reader can see in Fig. . Although the first criterion of FNN and $E_1$ of behave similiarly to the case $\tau=1$, the second test of FNN never reports anywhere near zero percent of FNNs, and $E_2$ statistic of AFN invariably osciales close to $1$, indicating nondeterminism. This is true both for original and surrogate data.

Also, we plotted the value of correlation dimension against $m$. The maximum agrees with the values of $m$ obtained by FNN and AFN. However, it does not saturate or even converge, but instead drops. (TODO: Why?)

\section{Largest Lyapunov exponents} \label{sec:lle-exp}
Largest Lyapunov exponent (LLE) was computed using the Rosenstein's algorithm \cite{Rosenstein1993} described in Section \ref{sec:rosenstein}. Fig. shows divergence plots for different values of the embedding dimension $m$ and time delay $\tau=1$. In Fig. , where $\tau=1$, we observe oscillations in the average divergence over time. This behavior is typical for noise \cite{Rosenstein1993}, but, as noted in section \ref{sec:rosenstein}, may occur due to measurement projection. Also, even if the largest Lyapunov exponent is positive, in dissipative systems (i.e. those possessing an attractor, see Section \ref{sec:attractor}) the sum of all Lyapunov exponents is negative, and thus, even on average, states will diverge in some directions (TODO: Add some comments about this to theoretical sections. Find citations.). These effects can be compensated for by using proper averaging statistics \cite{kantz2004}, and so we would expect Kantz's algorithm slightly alleviate this effect.

Moreover, we observe very similar behavior of the average divergence for the surrogate data, which, together with the observations made in previous sections, gives rise to the hypothesis of lack of chaos in the data. We tested this hypothesis statistically in Section \ref{sec:surrogate}.
