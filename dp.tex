...
\subsection{Frequency band amplitudes}
...
\chapter{Feature extraction approach}
...
\subsection{CSP analysis} \label{sec:csp}
...
\subsection{Filter Bank Common Spatial Patterns (FBCSP)} \label{sec:fbcsp}
...
\info[inline]{It was originally proposed for people with impeded motor control (e.g. disabled people) in context of BCI. But we show that similar method can be used to diagnose DEP and REMIS.}
...
We tried raw, cos sim, and recplots (with Chebyshev metric, because it seemed to show slightly more elaborate patterns, show image of difference). Our decision to try cos sim was because it is similar to GAF, but for nD-TS, and GAF was found to work. RP was used because it models the input as a non-linear system, and we can use some ``prior knowledge'' about the input this way. Another possibility is to learn on scalp images, but, as explained in \cite{schirrmeister2017deep}.
...
As mentioned in Chapter 1, brain is a non-linear dynamical system. Another possibility, therefore, is to represent the input data as recurrence plots, mentioned in Section . The obvious drawbacks are redundancy due to symmetry and high dimensionality. On the other hand, recurrence plots are known to capture properties of the system which are difficult to obtain using other methods in some cases. \cite{eckmann1987}

R
\add{Also point out to previous research with recurrence plots.}

Lastly, inspired by success of Gramian Angular Fields for sequence classification \cite{wang2015imaging}, we decided to try cosine similarity.

\subsection{Used architectures}
Explain how the CSP described above influences the proposed architectures. \cite{schirrmeister2017deep} Plots, number of parameters. We used ELUs, they worked better than ReLU, say what their benefits are.

\subsubsection{Shallow CSP-like network}

\subsubsection{Deep networks}
Here describe the two deep networks I used, how they differ (don't know yet).

% \subsubsection{Deep VGG-like network}

For recurrence plots and cosine similarities, we also tried the architecture (along with the same hyperparameters) used in \cite{garcia2018classification}, which resulted in overall accuracy of 0.942 and 0.804 recall on classification task of 6 activies using recurrence plots and CNNs, evaluated using 10-fold cross validation on over 10 000 samples. However, we were unable to replicate the result. This may be because of the difference in input image sizes, number of used input channels (the authors had only 4 electrodes available, and used all of them as input channels, whereas we used spatial embedding). 

\subsection{Dataset}
How we selected the datasets. Differences between previous chapter - need more data, so whole time series. 

\subsection{Preprocessing}
Filtering, running average and standard deviation.

\subsection{Results}
We used 5-fold cross validation. SHAL performs noticably better on REM than DEEP. Filtering has not much effect, but helps slightly for REM, but does not help on DEP.
\begin{table}[tbp]
\centering
\begin{tabular}{|c|c|c|c|c|}
\hline
...
Training & 3278 & 3230 & 5348 & 5442 \\ \hline 
Validation & 826 & 802 & 1349 & 1349 \\ \hline 
Test & 1038 & 997 & 1669 & 1704 \\ \hline 
Overall & 5142 & 5029 & 8366 & 8495 \\ \hline 
\end{tabular}
\caption{Negative / positive samples in training, validation, test sets.}
% \label{tab:depcl}
\end{table}
...
DEP & $0-f_{\text{fin}}$ & SHAL &  $84.53 \pm 1.29 \%$    \\ \hline 
 ...
 & $0-f_{\text{fin}}$ & DEEP &     $85.65 \pm 0.71 \%$    \\ \hline 
 & $4-f_{\text{fin}}$ & DEEP &     $84.63 \pm 2.02 \%$    \\ \hline 
REM & $0-f_{\text{fin}}$ & SHAL &  $86.11 \pm 0.38 \%$    \\ \hline 
 & $4-f_{\text{fin}}$ & SHAL &     $86.33 \pm 0.59 \%$    \\ \hline 
 & $0-f_{\text{fin}}$ & DEEP &     $83.87 \pm 2.24 \%$    \\ \hline 
 & $4-f_{\text{fin}}$ & DEEP &     $84.22 \pm 2.49 \%$    \\ \hline 
\end{tabular}
\subcaption{Caption.}
% \label{tab:depcl}
}
...
DEP & $0-f_{\text{fin}}$ & RP &  \\ \hline 
 & $4-f_{\text{fin}}$ & RP &  \\ \hline 
 & $0-f_{\text{fin}}$ & GAF &  \\ \hline 
 & $4-f_{\text{fin}}$ & GAF &  \\ \hline 
 & $0-f_{\text{fin}}$ & CS &  \\ \hline 
 & $4-f_{\text{fin}}$ & CS &  \\ \hline 
REM & $0-f_{\text{fin}}$ & RP &  \\ \hline 
 & $4-f_{\text{fin}}$ & RP &  \\ \hline 
 & $0-f_{\text{fin}}$ & GAF &  \\ \hline 
 & $4-f_{\text{fin}}$ &  GAF &  \\ \hline 
 & $0-f_{\text{fin}}$ & CS &  \\ \hline 
...
\add[inline]{How about seeing different activations?}
