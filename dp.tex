...
(TODO: Explain intuitively why this might work. Good: \cite{casdagli1991state})
...
This theorem, although of theoretical importance, is not necessarily useful in practice, since even dense sets can have measure zero. Moreover, it is restricted to smooth manifolds. An improvement came ten years later, when T. Sauer both generalized Takens' result as follows (in a simplified form):
...
A fascinating consequence of Theorem \ref{thm:sauer} when applied to a sequence of measurements recorded from a physical system is that a successfully reconstructed attractor does not describe the time series, but the system itself. In the words of Theiler: ``''

(Earlier, in 1936, Whitney showed that every $n$-dimensional differentiable manifold can be embedded in $\RR^{2n+1}$, and that the set of such embeddings is open and dense in the space of smooth maps. \cite{thm:whitney} \footnote{The second part of the theorem is a consequence of the fact that two hyperplanes with dimensions $d_1$ and $d_2$ in $m$-dimensional space are likely to intersect if $d_1 + d_2 \geq m$.})
...
(For highly chaotic systems with large Lyapunov exponents (see Section \ref{sec:lyap}) and large dimension, projection to a low dimensional time series causes explosion in the noise amplification. As a result, this imposes limits on short term predictablity and state space reconstruction may become impossible. Such systems should be treated as operationally stochastic. \cite{casdagli1991state})

(NOTES: Picking the correct embedding parameters $\tau$ and $m$ is a huge field. For highly chaotic systems, autocorrelation decays fast \cite{kantz2004}. Lower bound for $\tau$ is $1 - \frac{\sigma_{\text{noise}}}{\sigma_{\text{signal}}}$.)
A careful reader might have noted that the results of theorems in Section \ref{sec:method-of-delays} do not depend on the value of the delay $\tau$. Embeddings with the same value of the embedding dimension $m$, but different values of $\tau$ are theoretically equivalent. In practice, however, some theoretically sound time delay reconstructions may fail to be embeddings. For example, the embedding may fail to be a one-to-one map due to finite precision, or presence of noise in the data. Therefore, the proper choice of $\tau$ is of utmost importance. (Maybe partly due to its absence in the theoretical framework, no rigorous method of estimating $\tau$ exists. TODO: Before claiming this, read \cite{casdagli1991state} or their other papers.) Some researchers believe that the important parameter is the length of the embedding window $\tau_w = \tau (m-1)$.)
...
Hence, picking the proper value of $\tau$ is a balancing act between redundance and irrelevance.
...
Although this approach yields coordinates independent in a more general sense than simple linear independence provided by the autocorrelation function, the same criticism applies: minimum dependence between $s_t$ and $s_{t-\tau}$ says nothing about dependencies between other coordinates. Again, using this method is justifiable only for two-dimensional reconstructions. However, delayed mutual information has been generalized for multiple dimensions by its proponent A. M. Fraser \cite{fraser1989reconstructing} using multidimensional distributions into a concept he called \emph{redundancy}.
...
Moreover, both approaches described above address the issue of irrelevance, but not that of redundance. (This leads some authors to suggest another criterion to minimisation of mutual information. TODO: Should I talk about this?)

(TODO: Talk about criticisms. Other methods. How some people try to make the attractor as wide away from the diagonals as possible. Where have I seen it? The winow proponent paper.)

\subsection{The choice of embedding dimension}

(Reconstruction via the method of delays requires proper choice of parameters $m$ and $\tau$. From Theorem \ref{thm:takens} follows that vectors $\{\mathbf{s}_n\}$ are difeomorphic to the attractor $A$ of the system $M$ if $m$ is sufficiently large, specifically $m > 2d_A$ (hower, the reconstruction may be an embedding even for smaller $m$). On the other hand, $m$ adds redundancy and makes successive alogorithms less efficient (TODO: Elaborate.). Moreover, the total time delay $m\tau$ is limited by the recording time.)

There are multiple methods of choosing the parameters. We will start with a description of an algorithm for determining the minimal sufficient embedding dimension $m$, known as \textbf{false nearest neighbors} algorithm.
...
(For very small $\tau$, there is a there is a typical underestimation of m. Such a $\tau$ forces the attractor to lie near the diagonal in Rm. Increasing m by one has little effect on the geometry of the attractor as it will still lie near the diagonal of Rm+1. All the points will apparently look as true neighbors leading to a wrong conclusion. \cite{kugiumtzis1996state})
