...
\begin{center}
\textbf{\huge{}Non-linear dynamical analysis of EEG for depression diagnosis (using convolutional neural networks)}
...
\begin{center}
\textbf{\huge{}Nelineární dynamická analýza EEG pro diagnózu deprese (pomocí konvolučních neuronových sítí)}
\par\end{center}{\huge \par}
...
\noindent \textbf{Nelineární dynamická analýza EEG pro diagnózu deprese (pomocí konvolučních neuronových sítí)}
\end{onehalfspace}
...
\noindent \textbf{Non-linear dynamical analysis of EEG for depression diagnosis (using convolutional neural networks)}
\end{onehalfspace}
...
TODO: Attractors, Poincare plots, recurrence plots, Lyapunov coefficients, fractal dimension, Hurst exponent, etc.
...
(TODO: Other non-linear statistics that we will use: Lyapunov coeff., Higuchi's fractal dimension, DFA, sample entropy, Hurst exponents, correlation dimension, \dots)
...
\section{Applications in disease diagnosis}
...
In the same year, Hosseinifard et al. extracted Higuchi's correlation dimension, Lyapunov exponents and Higuchi's fractal dimension from 4 EEG channels of 90 patients split evenly between depressed and non-depressed subjects, achieving 90\% accuracy using a linear regression classifier. \cite{hosseinifard2013}
