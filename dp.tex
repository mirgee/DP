...
\begin{description}
  ...
  \item[\nameref{ch:concl}]~\\
    In the final chapter, the research objectives are reiterated and whether they have been met is discussed. Recommendations for future work are made, contributions and limitations of the work are examined.
\end{description}
...
\begin{dfn}[\cite{andreas2000}]
  Let $\mathbf{\xi} : \RR \rightarrow \RR^d$ be an $d \in \NN$ dimensional state (phase) space vector dependent on time, and $\mathbf{F}$ a smooth vector field in $\RR^d$. A \textbf{deterministic dynamical system}\footnote{In this work, we are going to assume that the brain is a deterministic dynamical system, and that any stochastic component is small and does not change nonlinear properties of the system. Thus, by the term dynamical system, we will always mean a deterministic dynamical system. This assumption is necessary for nonlinear dynamical analysis. On the other hand, nonlinear dynamical analysis also provides techniques (see Section \ref{sec:surrogate-analysis-theory}) which can partially adress (yet not fully answer) justification for this assumption. The question whether the brain is truly deterministic is open~\cite{stam2005, randomness}, but it is often thought to be probable that the brain is a nonlinear, deterministic, dissipative (i.e. exchanging energy with its environment) system~\cite{stam2005}.} is described by a set of $d$ first-order differential equations
  \begin{align*}
...
All the approaches described so far adress the issue of redundance by attempting to make the coordinates less correlated or expanding the reconstruction from the identity line. The issue of irrelevance however, needs to be adressed as well. In fact, based mostly on empirical, rather than theoretical grounds, most time delay estimation techniques optimize for the following criteria~\cite{kugiumtzis1996state}:
\begin{enumerate}
...
This technique by Cao~\cite{Cao1997} adresses one of the drawbacks of false nearest neighbors mentioned in the previous section - the variance of results based on subjective choice of embedding parameters. It does so by defining two parameter free functions dependent only on the embedding parameters.
...
A year later, Hosseinifard et al. extracted Correlation Dimension (CD), the Largest Lyapunov Exponent (LLE) and Higuchi's Fractal Dimension(HFD) from 4 EEG channels of 90 patients split evenly between depressed and non-depressed subjects, achieving 90\% accuracy using a logistic regression classifier and 3-fold leave-one-out cross validation~\cite{hosseinifard2013}.

In the same year, Bachmann et al. compared linear measure called Spectral Assymetry index (SASI) and HFD for depression diagnosis on 34 subjects split evenly between depressed and control group. SASI achieved true detection rate in 88\% in depressives and 82\% in the controls, while HFD provided true detection rate of 94\% in the depressives and 76\% in the controls~\cite{bachmann2013}, thus both giving comparable results.
...
\textbf{Measure} & \textbf{Analysis method} & \textbf{\spatcell{Accuracy or\\obtained results}} & \multicolumn{2}{c}{\textbf{Test sample}} \vline & \textbf{\# channels} & \textbf{Rec. length} & \textbf{Reference} & \textbf{Note} \\ \hline
 ...
 HFD & EPNN & 0.91 & 3 & 3 & 19 & 1 min & \cite{ahmadlou2012} & - \\ \hline
 HFD & Statistical analysis & 0.85 & 17 & 17 & 18 & 2 s & \cite{bachmann2013} & Evaluated on training set. \\ \hline
 ...
 \spatcell{Relative frontal\\$\theta$ power} & Statistical analysis & 0.63 & 45 resp. & 37 nonresp. & 4 & 2 s & \cite{iosifescu2009frontal} & \spatcell{Evaluated response\\to treatment.} \\ \hline
...
The dataset was provided by the \href{http://www.nudz.cz/en/}{Czech National Institute of Mental Health}, and the recording was performed by the institution's local specialists. It comprises of 133 subjects, 104 women and 29 men, ranging in age from 30 to 65 (47.7 $\pm$ 9.58).\footnote{We use the notation $\text{mean} \pm \text{std}$.} Handedness was not recorded. Montgomery-Asberg Depession Rating Scale (MADRS)~\cite{williams2008development} questionnaire assessed by a trained psychologist was used to measure depression severity. This psychometric measurement results in a depression score ranging from 0 (normal) to 40 (severe depression), usually with the following cutoff points~\cite{herrmann1998sunnybrook}: 
\begin{description}
...
Although the obtained accuracies are notably lower than those in studies we reviewed in Section \ref{sec:applications}, these results are not directly mutually comparable. Most importantly, all the reviewed studies employed a control group of symptomless, healthy patients, whereas our dataset composed entirely of patients suffering from depression with various degree of symptom severity. Nevertheless, the obtained accuracies are substantial, suggesting possible ``dose-response relantionship'' between depression severity and nonlinear measures recorded in EEG signals obtained from various cortical regions.
...
In this section, we aim to make some of essential terms used in this chapter more concrete by providing their mathematical definitions.
\begin{dfn}
...
Remind of the focus and objective
In this study, we incorporate both thorough review of the theory of nonlinear dynamical systems in relation to EEG analysis along with overview of the relevant algorithms and techniques for nonlinear measures extraction and input parameter estimation, comprehensive literature review, and practical application of those techniques on the available dataset. Furthermore, we also present another approach to feature extraction and classification on the dataset. 

\section*{Contributions}
% \begin{itemize}
% \item Performance on raw data considerably higher than on RP using DL - RP may not be as good as thought e.g. in \cite{acharya2015computer}
% \item F3 seemed relevant for response
% \item All examined NL measures are correlated with depression severity - the most significant correlation is in \ldots
% \item Used methods of estimating input parameters seem not very effective at fidning parameters whih would make the results discriminative
% \item FBCSP-inspired CNN models perform well especially for prediction 
% \item Neuroscience may be able to draw some conclusions from the observed correlation in brain regions
% \item We selected the input parameters very carefully, may inform other researchers about their proper selection (most studies just estimate them crudely)
% \end{itemize}
\subsection*{Analysis of Correlation between Depression Score and Nonlinear Measures}
Investignating the significance of correlations between nonlinear measures and depression scores, we found that LLE, DFA, HE, HD, and SE computed from EEG signals recorded from channels near cetain cortical areas are significantly correlated with depression score. The most statistically significant correlations were found for LLE, which, in \cite{hosseinifard2013} was found as least discriminative in a similar experimental setting. Although CD showed no significant correlations with depression score per channel, using it in combination with other nonlinear measures or by combining a number of channels resulted in one of the most accurate depression classifiers evaluated. For selection of channels relevant for classification, genetic algorithm resulted in substantially more accurate classifiers than any other feature selection method evaluated.

All the studies reviewed in Section \ref{sec:applications} included a control group of symptomless, clinically healthy patients, whereas our dataset consists exclusively of patients suffering from MDD of various symptom severity. Nevertheless, the accuracies achieved by the classifiers are substantial, indicating possible proportional relationship between depression severity and values of nonlinear measures recorded in EEG signals obtained from various cortical regions.

\subsection*{The Largest Lyapunov Exponents are Significantly Correlated with Positive Treatment Response}
\info[inline]{Will do this later, have to include review of treatment response, 4 studies. Add commentary to response results.}
Positively correlated with depression score, negatively with response. Thus LLE can be . We are not aware of any studies showing a similar result. Other nonlinear measures were not sign. corr., but still good feature for lin. classifiers. Although LLE showed significant correlation with depression score in temporal regions, frontal regions seemed to be more important for treatment response (F3 and T6 especially).

\subsection*{Analysis of Spatial Distribution of Nonlinear Measures across Brain Regions in Depression}
Although some neuroscientific theories suggest that medical depression is caused by malfuction in the right hemisphere \cite{hecht2010depression, kwon1996right}, in Sections \ref{sec:analdepdif} and \ref{sec:results-nl}, we found nonlinear measures computed in both hemispheres contribute significantly to high depression score. The distributions of correlation significance is notably particular to each measure, possibly indicating that variously distributed and different processes captured by each measure contribute to depression severity.\add{Too speculative? Belongs more to discussion?} For example, DFA and HE show most significant correlations with depression score in right parietal regions, LLE and SE in right temporal regions,  Nevertheless, the most successful depression classifiers made use especially of channels in temporal areas (in particular, T6 for depression). 

\subsection*{Analysis of Nonlinear Measure and Input Parameter Estimation Algorithms and Procedures for EEG Analysis}
Since computation of nonlinear measures requires laborious selection of input parameters and knowledge of the algorithms in order to interpret the results, these methods often remain inaccessible to practitioners who has not yet become experts in the field of nonlinear dynamical analysis \cite{schmid1996indications}. Therefore, there is a need for automated procedures to ease application of these methods and increase their use and availability. For the purpose of computing LLE and CD, in Section \ref{sec:estim-nl}, we evaluated a range of algorithms for estimation of embedding dimension and time delay parameters, including implementations of automated pipelines for computation of said measures. Although these input parameter estimation algorithms remain widely used (see Sections \ref{sec:lle-rev} and \ref{sec:corrdim-rev}), we found that, for out dataset, their outputs varied widely for each method, and provided only a crude approximation for parameters which produced more discriminative measures. One of the most consistent and informative estimation algorithms was ILD of our own implementation, which, however, suffers from relatively high computational costs.

\subsection*{Evaluation of FBCSP-inspired Neural Network Architectures for Depression Diagnosis and Prognosis}
We evaluated two CNN architectures which were previously shown to be successfully applied to decoding task-related information in pathology \cite{schirrmeister2017deep, schirrmeister2017deeppatho} on our dataset. Although the models achieved relatively high accuracy when trained and evaluated using cross validation across samples, when trained and evaluated using cross validation across recordings obtained before treatment from individual, distinct patients, the models achieved only modest accuracy, and did not generalize to the test set. This observation can be explained by low intrapersonal, and high intrapersonal variance of the EEG signals, which results in high correlation of samples corresponding to one patient, and thus in the presence of high number of samples from small number of patients, the model is trained to associate labels to samples based on the patient-charecterstic patterns, instead of label-characteristic patterns. This is a hypothesis supported by the literature, but needs to be tested. If it holds, then the performance of these models may be improved by extending the dataset with enough patients with similar patient-characteritic patterns and different label-characteristic patterns. Such dataset may be obtained easily by tracking patients' depression score across extended time span.
...
\item Better dataset
\item Filter 50 Hz
\item Try compression or encoding so that input to CNN can be longer (other solution voting)
\item Talk about a method of making more samples from less data from Robin Tibor Shirrmeister, how classification can be made by voting on shorter samples
\item Implement application to aid treatment
\item Try combining modalities
\item With larger dataset, more input parameters like kinds of treatment can be used
\item Correlations of activations with NL measures could be interesting - do CNNs extract some NL measures? Which ones?
\item Neuroscience: explain correlations in brain regions
...
\item Since we know that NL measures are useful for depression, architecture models inspired by the estimation algorithms or making use of NL algorithms may achieve high accuracy.
\end{itemize}
...
\end{tabular}
\caption[Mean values of LLE, before and after treatment comparison.]{Mean values of $\lambda_1$ of all patients before and after treatment.}
\label{tab:lleba}
\end{table}
...
\end{tabular}
\caption[Mean values of CD, before and after treatment comparison.]{Mean values of $D_2$ of all patients before and after treatment.}
\label{tab:d2ba}
\end{table}
...
\end{tabular}
\caption[Mean values of SE, before and after treatment comparison.]{Mean values of sample entropy of all patients before and after treatment.}
\label{tab:seba}
\end{table}
