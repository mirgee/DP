...
\begin{description}
  \item[preprocessing:] supressing undesirable distortions and noise, enhancement beneficient for further processing,
  \item[object segmentation:] separating disparate objects from the background,
...
In the description of the CSP algorithm, we follow the notatition from~\cite{blankertz2008optimizing}. Let $C$ be the number of channels, and $\mathbf{x}(t) \in \RR^C$ be a band-passed, de-meaned and scaled multichannel EEG recording. CSP analysis yields a projection of $\mathbf{x}(t)$ of the signal from the original signal space to $\mathbf{x}_{\text{CSP}}(t) \in \RR^C$ by finding a matrix $W \in \RR^{C \times C}$, where
\begin{align*}
