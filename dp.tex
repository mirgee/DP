...
Figure \ref{fig:acc-loss-fixed} shows loss and accuracy on the training and validation sets during one loop of the cross validation using the simpler, shallow model on the recordings obtained during the first session to predict the depression score level. In order to maximize the amount of training data, the thresholds for the classes were set to 26 as upper bound for healthy and 27 as lower bound for depressed. The figure shows that the model fits the training set of small number of patients, but does not generalize to the patients the patients in the validation set. Thus, the number of training epochs before evaluation on test set was decreased. Increasing the probability parameter of dropout, adding regularization to the layers, increasing or shortening the recording length to either increase the amount of information in a sample or simplify the model, or increasing the overlap between samples and shifting more patients to the training set does not improve generalization.
...
% Here add discossion of added table with accuracies. The table should contain: ww, ws, CV accuracy, 

\begin{table}[tbp]
\begin{tabular}{|l|l|l|l|l|l|}
\hline
\textbf{WW} & \textbf{WS} & \textbf{Train.} & \textbf{Val.} & \textbf{Test.} & \textbf{Acc.} \\ \hline
            &             &                 &               &                &               \\ \hline
            &             &                 &               &                &               \\ \hline
\end{tabular}
\caption[]{Table with accuracy as function of subrecording length, subrecording overlap, training, validation, test sizes.}
\end{table}

A possible explanation for the obsered result is as follows. It seems that the relatively high performance obtained in the previous section was mostly due to small intrapersonal variance and high interpersonal variance of the patterns in the EEG signals resulting in the network being trained to map patient-charactertic patterns to labels, instead of mapping label-characteresic patterns to labels. Hence, the models overfit over a small number of epochs to the training set of small number of patients, and is unable to generalize to another set of patients.

If this hypothesis holds, then the observed lack of generalization may be improved by increasing the number of patients included in the dataset. Assuming that the dataset would then contain large enough aggregate of patients with small variance in patient-charecterstic patterns and, at the same time, high enough variance in depression scores and treatment response, large training accuracy could be achieved by fitting the label-characterstic patterns. Conceivably, models trained on such dataset would generalize to patients not included in the training set.

\info[inline]{When we did the same, restricted only on predicting depression only on the before treatment set, we obtained slightly better results than chance.}
\section{Implementation}
For the implementation in this chapter, we used the same software as in Chapter \label{ch:nl}, which was listed in Section \ref{sec:impl-nl}. In additition, to design and evaluate the models, we used the deep learning library \href{https://keras.io/}{Keras}.
...
\subsection*{Identification of Ineffective Techniques}
