...
Computation of the Lyapunov spectrum for analyticial given $\mathbf{F}$ is straightforward using the definition above. But for dynamics given implicitly in a time series is difficult. It is commonly agreed that estimating Lyapunov exponents is even more difficult than esimating correlation dimension \cite{andreas2000}, although they have been successfully employed in EEG analysis. \cite{roschke1995, hosseinifard2013, stam2005} It has been claimed by P. Grassberger et al. that any application of these measures to physical systems should be interpreted with caution, mainly because all physical measurements are corrupted by noise, and reliable separation of signal is not always possible. \cite{grassberger1991nonlinear} They suggest that when emplying these techniques, the goal should not be to estabilish to strongest form of determinism, but to use them to ask whether determinism can be ruled out at all.
...
\add[inline]{Describe process of generating surrogate data.}
...
The results of the first four techniques were fairly consistent across channels and patients, with suggested delays usually in range 3-5. \add{Add precise statistics.} Note however, that those are the simpler techniques from our selection. The results of the other techniques were highly varied, yet usually estimate much higher values.

In the following, we will present an analysis of time series of $FP1$ electrode of patient one, trial one. 

Fig. \ref{fig:recon} shows reconstructed attractors for various time delay $\tau$. As expected, the reconstructed attractors for small delays cluster along the main diagonal, and get increasingly random with larger $\tau$. However, it is difficult to judge on the degree of folding in those reconstruction from these plots, mainly due to large number of data samples.

Typical plots of autocorrelation and delayed mutual information can be seen on Fig. \ref{fig:dmi-acorr} - similar behavior was observed across channels and patients. As expected from construction of the surrogate data, their plots look identical to those for original data. Delayed mutual information lacks perceptible minima, so this test yields no useful or reliable estimates of $\tau$. The red line on the autocorrelation plot shows the value $1-1/e$. 

Fig. \ref{fig:pca-svd} shows a convergence of the smaller singular values around $\tau=4$, in agreement with autocorrelation and mutual information. However, as we can observe in the same plot, the system is essentially one dimensional for $\tau \leq 6$ (this dimension probably corresponds to the main diagonal of the reconstructed attractor) and mostly two dimensional for $\tau \leq 16$. Similar behavior of two dominating singular values was observed, to some degree, throughout the dataset.

\unsure[inline]{Interestingly, however, this behavior was much less pronounced for some time series, usually marked by small $p$-values of the stationarity test. Moreover, for those time series, the results of these techniques were exceptionally consisent.}

The results of ADFD, see Fig. \ref{fig:adfd}, are usually consistent with the estimates provided by autocorrelation and delayed mutual information.

The result of ILD, the most powerful algorithm for estimation of the embedding parameters, be seen on Fig. \ref{fig:ild}. It is mostly featureless, however, the lines are slightly converging for $m \geq 8$. Also, there are some visible throughs, e.g. for $\tau=8$.

As explained in Section \ref{sec:delay}, these techniques should be used only as inspection tools, not as reliable guides for selection of $\tau$. The ultimate goal of the reconstruction is to obtain as accurate values of the non-linear parameters as possible, and thus selection of the optimal embedding parameters may differ for each of them.\add{Find some studies doing this also. Is there a way to justify this theoretically?} Thus, for example, in order to select the proper embedding parameters for computation of the largest Lyapunov exponent, we inspected the scaling regions for multiple values of $m$, $\tau$, Theiler window\add{Add description of Theiler window to \ref{sec:rosenstein}}, and other parameters, and picked those with the longest scaling regions (since the length of the scaling regions is proportional to the certainty of the estimate \cite{kennel1992determining}).
...
\noindent\makebox[\textwidth]{%
  ...
  \caption{Delayed mutual information and autocorrelation as functions of $\tau$. The red line shows threshold value for autocorrelation of $1-1/e$.}
\label{fig:dmi-acorr}
\end{figure}
...
\noindent\makebox[\textwidth]{%
  ...
  \caption{Plot of integral local deformation.}
\label{fig:ild}
\end{figure}
...
For estimating the embedding dimension, we used combination of False Nearest Neighbors (FNN) algorithm described in Section \ref{sec:fnn} and Average False Neighbors (AFN) described in Section \ref{sec:afn}.

After some experimentation, the parameters for FNN were $A_{\text{tol}} = 0.5$ and $R_{\text{tol}} = 3$.\add{Plot for different values.} The percentage of FNN's identified by the first te almost always plateaus for about $m=10$. The second test, however, is more sensitive to the tolerance value, and percentage of false neighbors reported by this test first quickly drops, and then starts raising. This behavior suggests noise corruption. \cite{kennel1992determining} As suggested by Kennel et al. \cite{kennel1992determining}, we used both criteria in conjuction to obtain the optimal value as suggested by FNN algorithm, and the results are significantly lower than those usually used in the literature for EEG data.\add{Report average $m$ computed this way, add citation.}

The $E_1$ statistic of AFN usually stops increasing for the same value as reported by FNN, thus the results of those two algorithms are in accordance. From Fig. \ref{fig:fnn-afn}, we can see that the $E_2$ statistic, which should be used for distinguishing stochastic and deterministic data \cite{Cao1997}, behaves almost identically for both original and surrogate time series'. This is an indication of nondeterminism of our data. 

However, this effect is amplified for $\tau > 1$, as the reader can see in Fig. \ref{fig:fnn-afn-osc}. Although the first criterion of FNN and $E_1$ of behave similiarly to the case $\tau=1$, the second test of FNN never reports anywhere near zero percent of FNNs, and $E_2$ statistic of AFN invariably oscillates around $1$, indicating nondeterminism. This is true both for original and surrogate data.

Also, we plotted the value of correlation dimension against $m$ for various values of $\tau$ (see Fig. \ref{fig:em-corrdim}). The maximum agrees with the values of $m$ obtained by FNN and AFN for $\tau=1$. However, it remains largely unchanged for higher values of $\tau$. Moreover, in contrast to expectation, it does not saturate or even converge, but instead drops. \unsure{Why?}

\begin{figure} 
\centering
\noindent\makebox[\textwidth]{%
  \includegraphics[width=1.0\textwidth]{Images/fnn_afn.png} }
  \caption{The results for False Nearest Neighbors and Average False Neighbors.}
\label{fig:fnn-afn}
\end{figure}
...
\centering
\noindent\makebox[\textwidth]{%
  \includegraphics[width=1.0\textwidth]{Images/fnn_afn_osc.png} }
  \caption{FNN and AFN for $\tau = 4$ using the same data as in Figure \ref{fig:fnn-afn}. }
\label{fig:fnn-afn-osc}
\end{figure}
...
\noindent\makebox[\textwidth]{%
  ...
  \caption{Correlation dimension for $\tau$ as function of the embedding dimension $m$.}
\label{fig:em-corrdim}
\end{figure}

\section{Largest Lyapunov exponents} \label{sec:lle-exp}
Largest Lyapunov exponent (LLE) was computed using the Rosenstein's algorithm \cite{Rosenstein1993} described in Section \ref{sec:rosenstein}. We chose Theiler window length of 10 (40 ms), as the results were similar for values 10, 50, 100, 1000.

Fig. \ref{fig:lle} shows divergence plots for different values of the embedding dimension $m$ and time delay $\tau=1$ for the original time series and surrogate data.

In Fig. \ref{fig:lle-osc}, showing the divergence plot of the original time series for $\tau=4$, we observe oscillations in the average divergence over time. This behavior is typical for noise \cite{Rosenstein1993}, but, as noted in section \ref{sec:rosenstein}, may occur due to measurement projection. Also, even if the largest Lyapunov exponent is positive, in dissipative systems (i.e. those possessing an attractor, see Section \ref{sec:attractor}) the sum of all Lyapunov exponents is negative, and thus, even on average, states will diverge in some directions (TODO: Add some comments about this to theoretical sections. Find citations.). These effects can be compensated for by using proper averaging statistics \cite{kantz2004}, and so we would expect Kantz's algorithm slightly alleviate this effect.

We observe very similar behavior of the average divergence for the surrogate data. This, together with the observations made in previous sections, gives rise to the hypothesis of lack of chaos in the data. We tested the hypothesis of linear Gaussian process in Section \ref{sec:surrogate-analysis}.

Supposing, in spite of the indications of nondeterminism, that it is reasonable to attempt compute the largest Lyapunov exponents, interpret and compare the obtained values, what is the optimal embedding dimension for this particular reconstruction? Using FNN and AFN as guides, $m=6$ is the optimal value. However, subjectively examining the divergence plot in Fig. \ref{fig:lle}, this choice of $m$ exhibits shorter scaling region than higher values of $m$, e.g. $m=10$. On the other hand, with higher embedding dimensions scaling region progressively disappears. Again, similar behavior is observed for most recordings - the scaling regions are longest for values of $m$ slightly higher than the one reported by FNN and AFN, plateauing for values $i < 15$.

Hence, based solely on length of the scaling regions, we might decide to use $m$ higher than those suggested by FNN and AFN. Notice that for $m=5$, i.e. for a value closed to the one suggested by FNN and AFN, the scaling region appears to be steeper, thus indicating more exponential rate of divergence, and is thus more consistent with the hypothesis of a chaotic system. For this reason, for the purpose of further analysis, we used $\tau=1$, values of $m$ suggested by FNN and AFN algorithms increased by one, and scaling regions of $i<15$.

\begin{figure} 
\centering
\noindent\makebox[\textwidth]{%
  \includegraphics[width=1.0\textwidth]{Images/lle.png} }
  \caption{Average divergence for $\tau=1$.}
\label{fig:lle}
\end{figure}
...
\centering
\noindent\makebox[\textwidth]{%
  \includegraphics[width=1.0\textwidth]{Images/lle_osc.png} }
  \caption{Average divergence for $\tau=4$.}
\label{fig:lle-osc}
\end{figure}
