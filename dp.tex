...
There are many subclassifications of nonlinearity, but for our purposes, we will use a sublass defined as follows.
...
EEG signals are prone to be infected with \emph{noise} due to imperfect isolation from surrounding environment. They are known to be \emph{nonstationary and nonlinear} \cite{kaplan2005} \cite{stam2005}. Since some patterns do not activate relative to a stimulus, a successful classifier must be able to detect a pattern regardless of its starting time, or find one. And finally, EEG records are relatively high dimensional - typical headsets contain 8-256 electrodes, which means that sampling at e.g. 256 Hz results in 2048-65536 data points per second.
...
\begin{dfn}[\cite{kantz2004}]
  ...
  \begin{align}
    E[|\mathrm{det} \mathbf{J}_{\mathbf{F}}|] < 1,
  \end{align}
...
For these systems, after sufficient passage of time, all future states will continue evolving on a bounded, time-invariant subset of $M$. This subset is a geometrical object called an \textbf{attractor}. Example of four basic attractors, point, limit, torus and chaotic attractors, can be seen in Figure \ref{fig:attractors}.
...
To this goal, let $\mathbf{s}_n$ be the reconstructed vector we are trying to find, and let us have a time series of scalar measurements of a quantity dependent on the current state of the system:
...
\begin{align}
  \textbf{x}_n = (x_n, x_{n+\tau}, x_{n+2\tau}, \ldots, x_{n+(m-2)\tau}, x_{n+(n-1)\tau}) \, ,
\end{align}
...
\begin{dfn}[\cite{kantz2004}]
  Let $x$ be a scalar time series of measurements $x_1, x_2, \cdots, x_N$. \textbf{Autocorrelation} $A : \RR \rightarrow \RR$ for time delay $\tau$ is given by
  \begin{align*}
...
\end{tabular}}
\caption[Literature review of $\lambda_1$ embedding parameters]{Embedding dimension $m$ and time delay $\tau$ choices found across available literature, along with methods of their selection and particular use case. All the studies used sampling frequency in range $250-256 Hz$.}
\end{table}
...
\end{tabular}}
\caption[Literature review of $D_2$ embedding parameters.]{Embedding dimension $m$ and time delay $\tau$ choices found across available literature, along with methods of their selection and particular use case. All the studies used sampling frequency in range $200-256 Hz$.}
\end{table}
...
\begin{landscape}
  ...
  \noindent\makebox[\textwidth]{%
    ...
    \caption[Comparison of mean $\lambda_1$ and $D_2$ between responders and non-responders with fixed parameters.]{Comparison of mean values of largest Lyapunov exponent and correlation dimension between responders and non-responders computed using embedding dimension $m=10$ and time delay $\tau=3$.}
   \label{fig:respnon1}
  \end{figure}
  ...
  \noindent\makebox[\textwidth]{%
    ...
    \caption[Comparison of mean $\lambda_1$ and $D_2$ between responders and non-responders with automatically selected parameters.]{Comparison of mean values of largest Lyapunov exponent and correlation dimension between responders and non-responders computed using automatic procedure described in Section .}
  ...
  \noindent\makebox[\textwidth]{%
    ...
    \caption[Comparison of mean DFA na SE between responders and non-responders.]{Comparison of mean values of computed detrended fluctuation analysis and sample entropy between responders and non-responders.}
   \label{fig:respnon2}
  \end{figure}
\end{landscape}
...
LLE, SE & SVM (lin.) & \textbf{0.75} $\pm$ 0.10 & 0.77 $\pm$ 0.09 & 0.75 $\pm$ 0.10 & 0.75 $\pm$ 0.10 & \makecell{\emph{LLE}: C4, T3, T6, Pz \\
                                                                                                         \emph{SE}: C3, P4} \\ \hline 
...
\hline
\textbf{Measure} & \textbf{Classifier} & \textbf{Accuracy} & \textbf{Precision} & \textbf{Recall} & \textbf{F-score} & \textbf{Channels} \\ \hline
LLE, SE & SVM (lin.) & \textbf{0.75} $\pm$ 0.10 & 0.77 $\pm$ 0.09 & 0.75 $\pm$ 0.10 & 0.75 $\pm$ 0.10 & \makecell{\emph{LLE}: FP2, F3, O1, T4, T6 \\
                                                                                                         \emph{SE}: F3, C3, T6 }  \\ \hline
LLE, CD& SVM (lin.) & \textbf{0.75} $\pm$ 0.11 & 0.76 $\pm$ 0.11 & 0.75 $\pm$ 0.11 & 0.75 $\pm$ 0.11 & \makecell{\emph{LLE}: F3, O2, T5, T6 \\
...
\caption{Evaluation of accuracies of the shallow (SHAL) and deep (DEEP) architectures on the raw and image-encoded data in classification of depression state (DEP) or prediction of future remission (REM).}
\end{table}
