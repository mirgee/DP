...
\info[inline]{Maybe the history should be presented first as an introduction. Or some broad definition, property, why we talk about it.}
\subsection{Mathematical Background}
In this section, we aim to make some of essential terms used in this chapter more concrete by providing their mathematical definitions. The first term requiring a definition is the term convolution, present in the name of the neural network class.
\begin{dfn}
...
\add[inline]{Change the definition, define both in one, improve description.}
When discussing the distinguishing properties of CNNs, we use the the terms invariance and equivariance.
\begin{dfn}
...
In other words, invariance to a transformation means that the if the transformation is applied to the input of the function, its output remains the sane. As for equivariance, if we view $G$ as a group of transformations, then equivariance is a commutative property of a function with regards to the transformations. In other words, computing the function and then applying the transformation has the same effect as applying the transformation and then computing the function.

A neural network can be viewed as a function approximation model based on and iterative optimization algorithm. There is a number of such algorithms in use, but many of them are variants of \textbf{gradient descent} algorithm. Gradient descent is a first order iterative method of finding an extremum a differentiable function $f : \RR^m \rightarrow \RR^n$, $f \in C^1$, based on continually moving a point in its domain space in the opposite direction of its gradient at that point, until the absolute value of the gradient is below a certain threshold. See Algorithm~\ref{alg:gd}.
