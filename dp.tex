...
We have been kindly provided by \href{http://www.nudz.cz/en/}{Czech National Institute of Mental Health} a dataset comprising of 266 multivariate EEG signal recordings of various durations and sampling rates obtained from 133 depressed patients, each recorded on two ocassions - before and, 4 weeks later, after drug administration. Before each recording, patient's mental health status was assessed by a trained professional using standardized questionnaire, and quantified into a depression score.\footnote{For more details about the dataset, see Section \ref{sec:dataset}.} Using these recordings and depression scores, we set out two goals:
...
\end{description}
For this purpose, we used two methods approaches shown to be relatively successful at this and similar tasks:
\begin{description}
  \item[nonlinear analysis approach]: compute nonlinear measures using nonlinear dynamical system analysis of the EEG signals, and use standard machine learning techniques to predict the patient's current or future depression level, and
...
\end{description}
% Maybe add a little detail about that we then evaluated the results and say add \ref{fig:diagram} 
\section*{Objectives}
